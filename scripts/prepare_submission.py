...
# get best bilateral psnr parameters from one image
path = rf'results\pso\0059_003_G4_00800_01000_5500_L_bilateral_psnr\results.csv'
df = pd.read_csv(path)
best = df.iloc[-1]
d = best['diameter']
sigma_color = best['sigmaColor']
sigma_space = best['sigmaSpace']
...
def my_srgb_denoiser(x):
    ...
    modified = cv.bilateralFilter(x, int(d), sigma_color, sigma_space)
    return modified
